var pokemonRepository = (function () {
  ...
  function add(pokemonObject) {
    if (
      ...
      (Object.keys(pokemonObject) === 'name' || 'height' || 'types')
    ) {
  ...
  function showModal(pokemon) {
    ...
    var typesPokemon2 = $(
      '<p id="' + pokemon.types[1] + '">' + pokemon.types[1] + '</p>'
    )

    // Add the image animation with changing colors by types
    var imgContainer = $('<div id="img-container"></div>')
    var imgPokemon = $(
      '<div class="image"><img src="' + pokemon.imageUrl + '"></div>'
    )
    var imgCircle = $('<div class="image-circle"></div>')
    imgCircle.addClass(pokemon.types[0])
    var imgCircle0 = $(
      '<div class="image-circle" style="animation-delay: -3s"></div>'
    )
    imgCircle0.addClass(pokemon.types[0])
    var imgCircle1 = $(
      '<div class="image-circle" style="animation-delay: -2s"></div>'
    )
    imgCircle1.addClass(pokemon.types[0])
    var imgCircle2 = $(
      '<div class="image-circle" style="animation-delay: -1s"></div>'
    )
    imgCircle2.addClass(pokemon.types[0])
    var imgCircle3 = $(
      '<div class="image-circle" style="animation-delay: 0s"></div>'
    )
